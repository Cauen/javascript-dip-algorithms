...
function getCodedImageHtml(imageData) {
    ...
    var randomValueR = (imageData.data[imageData.data.length - 4] == (imageData.data[0] - 85)) ? imageData.data[imageData.data.length - 4] : false;
    var randomValueG = (imageData.data[imageData.data.length - 3] == (imageData.data[1] - 31)) ? imageData.data[imageData.data.length - 3] : false;
    var randomValueB = (imageData.data[imageData.data.length - 2] == (imageData.data[2] - 20)) ? imageData.data[imageData.data.length - 2] : false;
    if (randomValueR !== false && randomValueG !== false && randomValueB !== false) // Can be 0
    	console.log('Coded Image');
    else {
    	noty({
			text: 'Not coded image',
			type: 'error',
			layout: 'topRight',
			timeout: 1500
		});
